...
data_list = list()
for(i in 1:4){
    data_list[[i]] = read.xlsx('./data/GDP.xls', i, encoding = 'UTF-8')}

m1 = as.matrix(data_list[[1]])   # 第一列为年份
m2 = as.matrix(data_list[[2]])   # 第一列为北京
m3 = as.matrix(data_list[[3]][,-34])   # 第一列为年份
m4 = as.matrix(data_list[[4]])   # 第一列为北京

gdp = rbind(m1, m2, m3, m4)
